...
def prepare_set(N_FEATURES: int, instance: Instance, sol_cont):
	...
	"""
	X = np.zeros((instance.n_items, N_FEATURES))
	for i in range(instance.n_items):
			positive_syn, negative_syn = countSynergies(str(i), instance.polynomial_gains)
			X[i, 0] = sol_cont[i]
			X[i, 1] = instance.profits[i]
			X[i, 2] = instance.costs[i][0]/instance.budget
			X[i, 3] = instance.costs[i][1]/instance.budget
			X[i, 4] = positive_syn
			X[i, 5] = negative_syn
	return X
